...
document.querySelector("#btn9").addEventListener("click", function() {
	...
	.then((response) => response.text()) //fetch된 값을 받음
...
document.querySelector("#btn8").addEventListener("click", function() {
	...
	.then(b => console.log(b * 2)) // ? log를 찍은 값이 리턴 -> 다른 값이 나옴 12xx
...
document.querySelector("#btn3").addEventListener("click", function(){
		fetch(ctx + "/ex46/sub01")
		.then(function(){ //순서대로 실행시키는 코드
		console.log("첫번째 then 함수");
		return "abc"; // then 함수가 데이터 전달 가능
		})
			.then(function(data){
...
document.querySelector("#btn2").addEventListener("click", function(){
	fetch(ctx + "/ex46/sub01").then(function(){ //순서대로 실행시키는 코드
