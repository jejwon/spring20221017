...
<body>
	...
	<input type="number" id = "idInput1" placeholder="아이디"> <br>
	<input type="text" id = "nameInput1"  placeholder="이름"> <br>
	<input type="text" id = "cityInput1"  placeholder="도시"> <br>
	<input type="text" id = "countryInput1"  placeholder="국가"> <br>
	...
	<button id="btn13">/ex47/sub13 post 요청 : 공급자정보 insert</button><br>
 	<input type="number" id="supplierIdInput13" placeholder="아이디"> <br>
	...
	<br>
	<button id="btn15">/ex47/sub14 delete 요청 : 공급자정보 delete</button>
	<input type="number" id = "supplierIdDelete15">
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-OERcA2EqjJCMA+/3y+gxIOqMEjwtxJY7qPCqsdltbNJuaOe923+mo//f6V8Qbsw3" crossorigin="anonymous"></script>
<script>
const ctx = "${pageContext.request.contextPath}";

document.querySelector("#btn15").addEventListener("click", function() {
	const id = document.querySelector("#supplierIdDelete15").value;
...
document.querySelector("#btn11").addEventListener("click", function(){
	...
	document.querySelector("#idInput1").innerText = "";
	document.querySelector("#nameInput1").innerText = "";
	document.querySelector("#cityInput1").innerText = "";
	document.querySelector("#countryInput1").innerText = "";
	
	const id =  document.querySelector("#idInput1").value; //값을 받기
	const name =  document.querySelector("#nameInput1").value;
	const city = document.querySelector("#cityInput1").value;
	const country = document.querySelector("#countryInput1").value;
	  
	const data = {id, name, city, country}; //객체에 넣었으니까 그대로 써도 됨
...
document.querySelector("#btn7").addEventListener("click", function() {
	const customerId = document.querySelector("#customerIdInput1").value; //pathvariable 요청 경로에 정보가 포함됨
	fetch(ctx + "/ex47/sub07/" + customerId)
	.then(res => res.json())
	.then(sup => {
...
document.querySelector("#btn1").addEventListener("click", function() {
	fetch(ctx + "/ex47/sub01")
	.then((res) => res.json())
	.then((data) => console.log(data)); //json string이 아닌 객체로 리턴
}); 
</script>
</body>
</html>
